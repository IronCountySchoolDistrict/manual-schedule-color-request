...
(function () {
    'use strict';
    var template = $j($j('#color-key-template').html());
    var select = $j('.box-round');
    ...
    var numRows = $j('.box-round tr').length;

    //Loop through the table rows
    for (var i = 3; i < numRows; i++) {
        var courseNum = $j('.box-round tr')[i].children[0].innerHTML;
        ...
        for (var j = 0; j < numRequests; j++) {
            ...
            if (requests[j].course === courseNum) {
                ...
                if (requests[j].type === '') {
                    $j('.box-round tr')[i].children[0].style.setProperty('background-color', '#00f000', '');
                    $j('.box-round tr')[i].children[0].innerHTML += '&nbsp;&nbsp;(' + requests[j].priority + ')';
                }
                //It's an ELECTIVE (non-alt) -- color it yellow
                else if (requests[j].type === 'E' && requests[j].alternate === 0) {
                    $j('.box-round tr')[i].children[0].style.setProperty('background-color', '#f0f000', '');
                    $j('.box-round tr')[i].children[0].innerHTML += '&nbsp;&nbsp;(' + requests[j].priority + ')';
                }

                //It's an ALT ELECTIVE -- color it red
                else if (requests[j].type === 'E' && requests[j].alternate === 1) {
                    $j('.box-round tr')[i].children[0].style.setProperty('background-color', '#f00000', '');
                    $j('.box-round tr')[i].children[0].innerHTML += '&nbsp;&nbsp;(' + requests[j].priority + ')';
                }
            }
        }
